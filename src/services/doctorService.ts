...
class DoctorService {
  ...
  async getMyPatients(page: number = 1, limit: number = 10): Promise<PaginationResponse<Patient>> {
  ...
  async getDoctorStats(): Promise<DoctorStats> {
    const response = await apiService.get<ApiResponse<DoctorStats>>('/doctor/stats');
